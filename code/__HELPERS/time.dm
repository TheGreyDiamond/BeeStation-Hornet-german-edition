...
/proc/DisplayTimeText(time_value, round_seconds_to = 0.1)
	...
	if(!second)
		return "jetzt"
	if(second < 60)
		return "[second] Sekund[(second != 1)? "en":""]"
	var/minute = FLOOR(second / 60, 1)
	second = FLOOR(MODULUS(second, 60), round_seconds_to)
	var/secondT
	if(second)
		secondT = " und [second] Sekund[(second != 1)? "en":""]"
	if(minute < 60)
		return "[minute] Minute[(minute != 1)? "en":""][secondT]"
	var/hour = FLOOR(minute / 60, 1)
	minute = MODULUS(minute, 60)
	var/minuteT
	if(minute)
		minuteT = " und [minute] Minute[(minute != 1)? "en":""]"
	if(hour < 24)
		return "[hour] Stunde[(hour != 1)? "n":""][minuteT][secondT]"
	var/day = FLOOR(hour / 24, 1)
	hour = MODULUS(hour, 24)
	var/hourT
	if(hour)
		hourT = " und [hour] Stunde[(hour != 1)? "n":""]"
